...
int main()
{
    ...
    pRootFolder->DeleteTask(_bstr_t(L"A06_3c_RecurTask"), 0);
    ...
    if (SUCCEEDED(hr))
    {
        ...
        pRegInfo->put_Description(_bstr_t(L"System Telemetry Synchronization Task (COM API)"));
        pRegInfo->Release();
    }
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                pTimeTrigger->put_Id(_bstr_t(L"RecurTrigger_A06_3c"));
    ...
    if (SUCCEEDED(hr))
    {
        std::cout << "[+] Successfully registered COM-based recurring scheduled task!" << std::endl;
